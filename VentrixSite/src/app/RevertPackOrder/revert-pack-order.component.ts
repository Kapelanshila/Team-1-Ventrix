...
@Component({
  selector: 'app-revert-pack-order',
  templateUrl: './revert-pack-order.component.html',
  styleUrls: ['./revert-pack-order.component.css']
})
export class RevertPackOrderComponent implements OnInit {
  p: number = 1;
  config: any; 
  noOfRows = 10;
  //Search query 
            ...
            response.forEach(element => {
             if (element.clientId == clientorder.clientId)
             {
              ...
              .subscribe(response => {
                ...
                if (this.temp.description == "Packed")
                {
                  this.order = 
                  {
  ...
  revertPackOrder(selectedClientOrder: ClientOrderVM)
  {
    Swal.fire({
      icon: 'question',
      title: 'Are you sure you want to undo this change?',
      showDenyButton: true,
      ...
      if (result.isConfirmed) {
        Swal.fire({
          icon: 'success',
          title: 'Changes have been reverted',
          ...
          if (result.isConfirmed) {
            this.ventrixdbservice.revertPackOrder(selectedClientOrder).subscribe();
            this.router.navigate(['/revert-pack-order']).then(() => {
  ...
  {
    this.ventrixdbservice.createOrderPDF(selectedClientOrder)
    .subscribe(res => {
      const data = new Blob([res] , { type: 'application/pdf' });
     saveAs(data, selectedClientOrder.description.toString()+" Pack Order Report");
   });
  }
  ...
  addInventory(selectedClientOrder: ClientOrderVM)
  {
    ...
    else
    {
      ...
      this.router.navigate(['/pack-orderline']);  
    }
  }
