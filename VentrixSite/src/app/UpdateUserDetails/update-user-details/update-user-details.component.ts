...
export class UpdateUserDetailsComponent implements OnInit {
  ...
  constructor(fbuilder: FormBuilder, private router: Router,private ventrixdbservice:VentrixDBServiceService)
  {
      ...
      this.userdetailsform = fbuilder.group({
  ...
  updateUserDetails()
    ...
    this.updatedemployee.forEach(element => {
      if(element.idnumber==this.userdetailsform.get('idNumber')?.value && element.idnumber != "0101105153080")
        {
          this.find = true;
          Swal.fire({
            icon: 'error',
            title: 'User Details Already Exists',
            ...
            if (result.isConfirmed) {
              this.router.navigate(['/view-user-details']).then(() =>{
      ...
      if(this.userdetailsform.valid && this.find == false)
      {
        this.ventrixdbservice.updateEmployee(this.userdetailsform.value).subscribe();
